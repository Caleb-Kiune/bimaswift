...
import { DetailedQuoteBreakdown, InsuranceProduct } from "@/src/features/motor-private/types";
...
interface PrivateQuoteCardProps {
  insurerId: string;
  insurerName: string;
...
export default function PrivateQuoteCard({
  insurerId,
  insurerName,
  quote,
  riderIds = [],
  ...
  return (
    <div className="relative bg-white rounded-2xl border border-zinc-200 hover:border-zinc-300 transition-all duration-200 shadow-sm">
      <button
      ...
      >
          ...
          <div>
            ...
            {quote.vehicleValue && (
              <span className="text-xs text-zinc-500 mt-1 block">
                Sum Insured: KES {quote.vehicleValue.toLocaleString()}
              </span>
            )}
            <div className="flex flex-wrap items-center gap-2 mt-1">
              <span className="text-xs text-zinc-500">
                {quote.coverType === "COMPREHENSIVE" || displayedCoverType === "COMPREHENSIVE"
                  ? "Comprehensive"
                  : "Third Party Only"}
              </span>
              {quote.calculatedRiders.filter(
                (r) => r.premium.value === 0,
              ).length > 0 && (
        ...
        <div className="mt-3 sm:mt-0 w-full sm:w-auto flex flex-row sm:flex-col justify-between sm:items-end items-center">
          <span className={`font-extrabold text-2xl tracking-tight text-zinc-900 transition-opacity duration-300 ${isRecalculating ? "opacity-30" : "opacity-100"}`}>
            KES {quote.totalPayable.toLocaleString("en-KE")}
          </span>
      ...
      {isExpanded && (
        <div className="px-5 pb-5">
          <div className="border-t border-dashed border-zinc-300 pt-4 text-sm">
            ...
            <div className="mb-3">
              <div className="flex justify-between items-center text-zinc-600">
                ...
                <span className="font-semibold text-zinc-900">
                  {formatKES(quote.basicPremium)}
                </span>
              </div>
              <div className="flex gap-2 mt-1">
                <span className="text-[10px] text-zinc-400">
                  Rate: {formatRate(quote.basicPremium.breakdown.rateBps)}
                </span>
                {quote.basicPremium.breakdown.isMinPremiumApplied && (
            ...
            {/* --- 2. RIDERS BREAKDOWN --- */}
            {quote.calculatedRiders.length > 0 && (
                  ...
                  <div key={rider.id} className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-[10px] text-zinc-400 ml-5 mt-0.5">
                        {rider.premium.breakdown.rateType === "FREE" && "Complimentary"}
                        ...
                        {rider.premium.breakdown.isMinPremiumApplied && " (Min. Applied)"}
                      </p>
                    </div>
                    <span className="text-xs font-medium text-zinc-700">
                      {rider.premium.value === 0 ? (
                        <span className="text-emerald-600 font-semibold text-xs uppercase">Free</span>
                      ) : (
            ...
            <div className="text-sm border-t border-dashed border-zinc-200 pt-3 space-y-1.5">
              <div className="flex justify-between text-xs text-zinc-500">
                <span>Training Levy ({formatRate(quote.itl.breakdown.rateValue)})</span>
                <span>{formatKES(quote.itl)}</span>
              </div>
              <div className="flex justify-between text-xs text-zinc-500">
                <span>Policyholders Fund ({formatRate(quote.phcf.breakdown.rateValue)})</span>
                <span>{formatKES(quote.phcf)}</span>
              </div>
              <div className="flex justify-between text-xs text-zinc-500">
                <span>Stamp Duty</span>
                <span>{formatKES(quote.stampDuty)}</span>
              </div>
            </div>
            ...
            <div className="pt-3 mt-3 border-t border-zinc-200">
              <p className="flex justify-between items-baseline">
                <span className="text-sm font-semibold text-zinc-700">Total Premium</span>
            ...
            {/* UPGRADES SECTION - Only show if not in History view */}
            {!isHistoryView && displayedCoverType === "COMPREHENSIVE" && specialtyRiders.length > 0 && (
              <div className="mt-4 pt-4 border-t border-zinc-200 bg-zinc-50 rounded-xl p-4">
                ...
                <div className="space-y-5">
                  {specialtyRiders.map((rider) => {
                    const isSelected = !!insurerUpgrades[rider.type];
                    const currentOption = (typeof insurerUpgrades[rider.type] === "string" 
                      ? insurerUpgrades[rider.type] 
                      : rider.options?.[0]?.id || "") as string;

                    return (
                      <div key={rider.id} className="flex flex-col space-y-2">
                        <label className="flex items-center justify-between cursor-pointer group">
                          ...
                          <div className="relative inline-flex items-center">
                            <input
                              ...
                              onChange={(e) => {
                                e.stopPropagation();
                                handleInsurerRiderToggle?.(insurerId, rider.type);
                                if (!isSelected && rider.options && rider.options.length > 0) {
                                  handleInsurerRiderOptionChange?.(insurerId, rider.type, rider.options[0].id);
                                }
                              }}
                            />
                        ...
                        {isSelected && rider.options && rider.options.length > 0 && (
                          <div className="ml-2 pl-3 border-l-2 border-indigo-200 transition-all duration-300">
                            <CustomSelect
                              options={rider.options}
                              value={currentOption}
                              onChange={(newOptionId) => handleInsurerRiderOptionChange?.(insurerId, rider.type, newOptionId)}
                            />
                          </div>
                        )}
                      </div>
            ...
            <div className="pt-4 space-y-3">
              <DownloadPrivateQuoteWrapper insurerName={insurerName} quote={quote} />
